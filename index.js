function superbowlWin(functionArray){
  let latestWin = {};
    functionArray.forEach(season => {
    if(season.result != "W"){
      sadReality.push({year:season.year, result:undefined})
    }else{
      latestWin = functionArray.find(season => season.result === "W");
    };
  })
  console.log(sadReality);
  console.log(latestWin);
  return latestWin.year;
}
